...
private:
  ...
  void update_point_cloud_rgb()
  {
    ...
    if (show1_ && laser1_)
    {
      ...
      try
      {
        transform_stamped = tf_buffer_->lookupTransform("base_link", laser1_->header.frame_id, tf2::TimePointZero);
      }
      catch (tf2::TransformException &ex)
      {
        RCLCPP_WARN(this->get_logger(), "Could not transform %s to target_frame: %s", laser1_->header.frame_id.c_str(), ex.what());
        return;
      }
      ...
      for (float i = temp_min_; i <= temp_max_ && count < laser1_->ranges.size(); i += laser1_->angle_increment)
      {
        ...
        tf2::Vector3 tf2_point(pt.x, pt.y, pt.z);
        tf2_point = tf2_transform * tf2_point;
        pt.x = tf2_point.x();
        pt.y = tf2_point.y();
        pt.z = tf2_point.z();

        if (std::sqrt(std::pow((pt.x), 2) + std::pow((pt.y), 2)) < 1.0)
          continue;
    ...
    if (show2_ && laser2_)
    {
      ...
      try
      {
        transform_stamped = tf_buffer_->lookupTransform("base_link", laser2_->header.frame_id, tf2::TimePointZero);
      }
      catch (tf2::TransformException &ex)
      {
        RCLCPP_WARN(this->get_logger(), "Could not transform %s to target_frame: %s", laser1_->header.frame_id.c_str(), ex.what());
      ...
      for (float i = temp_min_; i <= temp_max_ && count < laser2_->ranges.size(); i += laser2_->angle_increment)
      {
        ...
        //std::cout << "Before x: " << pt.x << " y: " << pt.y << " z: " << pt.z << std::endl;
    ...
    pc2_msg_->header.frame_id = "base_link"; // Update the frame_id to the target frame
